...
- Verified workspace customization path:
  ...
  - when migrating older Degrande state from plugin settings or local storage into graph-backed DB properties, do not perform those writes inline in the initial startup load; queue them and flush them after indexing through the existing sync refresh path

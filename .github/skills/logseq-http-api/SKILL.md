...
- Verified workspace customization path:
  ...
  - keep a manual sync action in the Degrande panel, but avoid timer or interval based sync polling; prefer Logseq event-driven hooks and explicit manual sync so the status indicator does not sit in a false in-progress state

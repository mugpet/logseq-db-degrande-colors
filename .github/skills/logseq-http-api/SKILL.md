...
- Verified workspace customization path:
  ...
  - prefer a dedicated graph-backed sync revision property over timer-based polling for Degrande state; update that shared revision value on each graph-backed write and react to revision changes through Logseq events

...
- Verified workspace customization path:
  ...
  - for graph-backed controls, gradients, revisions, and page-backed migration reads in `logseq-db-degrande-colors`, prefer query-first DB reads in DB mode and avoid `Editor.getBlockProperty` fallback during early startup; the fallback can emit noisy `get-block error` failures before indexing completes
  - when migrating older Degrande state from plugin settings or local storage into graph-backed DB properties, keep the direct migration path but suppress recoverable startup write errors; do not queue stale migration state in memory where it can overwrite newer edits later
  - treat `v0.1.34` as the current working save/sync baseline for Degrande appearance state; `v0.1.33` deferred those migrations in memory and regressed visible save/sync behavior, so do not reintroduce that pattern

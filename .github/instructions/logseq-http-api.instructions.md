...
- In `logseq-db-degrande-colors`, do not write legacy migration state into graph-backed properties inline during the initial startup load. Queue legacy settings/local-state migrations and flush them from the existing post-index or startup-refresh sync path once the graph is writable.

...
- In `logseq-db-degrande-colors`, prefer a dedicated graph-backed sync revision property over timer-based polling for Degrande state. Update that shared revision value on every graph-backed Degrande write, and let Logseq event hooks react to the revision change.

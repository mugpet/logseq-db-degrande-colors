...
- Keep a manual graph-sync action in the Degrande panel, but avoid timer or interval based sync polling. Prefer Logseq event-driven hooks such as `DB.onChanged`, `App.onGraphAfterIndexed`, and explicit manual sync over background polling that can misreport state and add needless work.
- In the Degrande panel, prefer a compact sync-status indicator over a full sync button: green when the last sync pass completed, yellow when graph sync is pending or in progress, and tooltip text inviting manual sync on click.

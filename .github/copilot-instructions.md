...
- Verified published-plugin styling fix: packaged installs can fail to reach the host document directly even when the unpacked plugin can. Keep the host `<style>` management path for accessible/unpacked contexts, but preserve the `logseq.provideStyle` fallback for packaged installs so tag and gradient CSS still lands in the app document.
...
- Verified DB persistence rule for `logseq-db-degrande-colors`: plugin settings are a plugin-user settings channel, not graph-native synced state. For Degrande state that must persist with a Logseq DB graph, keep controls and gradients in graph-backed DB properties and keep tag colors on the actual tag/page entities. Use plugin settings only as a migration source from older releases.
...
- For `logseq-db-degrande-colors`, do not open or prepare marketplace PRs unless the user explicitly asks for that path. The normal publish path is updating the repo, bumping the version, and pushing the corresponding release tag.
- For `logseq-db-degrande-colors` persistence work in Logseq DB mode, do not treat `logseq.updateSettings` as the primary sync/persistence target for graph-owned appearance state. Controls and gradients should be loaded from and saved to graph-backed DB properties, while tag colors should remain on the relevant tag/page entities.
- For Degrande styling updates, keep one managed host `<style>` element when the host document is accessible. For packaged installs where that is not reliable, preserve the `logseq.provideStyle` fallback instead of removing it.
- For packaged-style fallback updates, emit neutral per-tag reset rules for known tags as well as active custom rules. Otherwise stale published tag rules can survive after a color reset.
